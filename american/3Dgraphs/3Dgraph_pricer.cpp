/* This version of the pricer computes the grid for an American put and call
and prints them to separate text files */
...
int main()
{
    ...
    call_grid.open("call_grid.txt");
    if (!call_grid) {
        cerr << "Unable to open call_grid.txt. \n";
        exit(1);
    }

    put_grid.open("put_grid.txt");
    if (!put_grid) {
        cerr << "Unable to open put_grid.txt. \n";
        exit(1);
    }

    float params[11] = {0, 1, 0, 100, 120, 0.5, 0.2, 0.1, 0.05, 500, 500};

    Matrix<float> call = fdm(params);
    for (int i=0; i<500; i++) {
        for (int j=0; j<500; j++) {
            call_grid << call(i, j) << ", ";
        }
