...
and outputs the parameters and the European, then American prices to
to AMgeqEUR.txt */
...
#include <iomanip>
#include <time.h>
#include "AMgeqEUR_pricer.h" // defines Matrix template to avoid seg fault
...
int main()
{
    ...
    ofstream AMgeqEUR;
    AMgeqEUR.open("AMgeqEUR.txt");

    // check if AMgeqEUR.txt was opened successfully
    if (!AMgeqEUR) {
        std::cerr << "Unable to open AMgeqEUR.txt. \n";
        exit(1);
    }
    ...
    while (options.peek() != '*') {
        ...
        // the parameters and a null character to AMgeqEUR.txt
